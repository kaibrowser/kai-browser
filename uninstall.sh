...
[ -f "$BIN_DIR/kaibrowser" ] && rm "$BIN_DIR/kaibrowser" && echo "✓ Removed executable"
...
for SHELL_CONFIG in "$HOME/.bashrc" "$HOME/.zshrc"; do
    if [ -f "$SHELL_CONFIG" ]; then
        if grep -q "# Added by Kai Browser installer" "$SHELL_CONFIG"; then
            sed -i '/# Added by Kai Browser installer/d' "$SHELL_CONFIG"
